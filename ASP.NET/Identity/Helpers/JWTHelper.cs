    ...
    public class JWTHelper : IJWTHelper
    {
        ...
        private List<Claim> GenCliams(JWTCliam jwtCliam, List<string> roles)
        {
            ...
            var claimMappings = new Dictionary<string, string>
            {
                ...
                { nameof(jwtCliam.name), ClaimTypes.Name },
            ...
            {
                var value = jwtCliam.GetType().GetProperty(mapping.Key)?.GetValue(jwtCliam, null) as string;
        ...
        public RunStatus GetJWT(JWTCliam jwtCliam, List<string> roles)
        {
            ...
            string secretKey = _jwtConfig.SignKey; // 用於加密的密鑰
            string issuer = _jwtConfig.Issuer; // Token 的發行者
            int expireMinutes = _jwtConfig.ExpireDateTime; // Token 的過期時間（分鐘）
